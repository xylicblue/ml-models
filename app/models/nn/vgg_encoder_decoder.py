"""VGG16-BN Encoder-Decoder for plant counting and size estimation.

Ported from plant_counting_-_size_estimation_gradio/models.py.
Architecture unchanged — required for loading existing checkpoint weights.
"""
...
class VGG(nn.Module):
    """VGG16 encoder with batch normalization. Returns multi-scale features."""
...
class BackEnd(nn.Module):
    """Decoder with skip connections for density map prediction."""
...
class Detect(nn.Module):
    """Global average pooling branch for size estimation (scalar output)."""
...
class Model(nn.Module):
    """Main model: VGG encoder + density decoder (gap=False) or size estimator (gap=True).

    gap=False: Counting model — outputs (density_map, attention_map)
    gap=True:  Size model — outputs scalar size estimate
    """

    def __init__(self, gap=False, load_pretrained_vgg=True):
        super().__init__()
        self.gap = gap
        self.vgg = VGG()
    ...
    def load_vgg(self):
        """Load pretrained VGG16-BN weights, remapping layer names."""
        state_dict = model_zoo.load_url(
            "https://download.pytorch.org/models/vgg16_bn-6c64b313.pth",
            progress=False,
        )
        old_name = [0, 1, 3, 4, 7, 8, 10, 11, 14, 15, 17, 18, 20, 21,
                    24, 25, 27, 28, 30, 31, 34, 35, 37, 38, 40, 41]
        new_name = ["1_1", "1_2", "2_1", "2_2", "3_1", "3_2", "3_3",
                    "4_1", "4_2", "4_3", "5_1", "5_2", "5_3"]
        new_dict = {}
        for i in range(13):
            new_dict[f"conv{new_name[i]}.conv.weight"] = \
                state_dict[f"features.{old_name[2 * i]}.weight"]
            new_dict[f"conv{new_name[i]}.conv.bias"] = \
                state_dict[f"features.{old_name[2 * i]}.bias"]
            new_dict[f"conv{new_name[i]}.bn.weight"] = \
                state_dict[f"features.{old_name[2 * i + 1]}.weight"]
            new_dict[f"conv{new_name[i]}.bn.bias"] = \
                state_dict[f"features.{old_name[2 * i + 1]}.bias"]
            new_dict[f"conv{new_name[i]}.bn.running_mean"] = \
                state_dict[f"features.{old_name[2 * i + 1]}.running_mean"]
            new_dict[f"conv{new_name[i]}.bn.running_var"] = \
                state_dict[f"features.{old_name[2 * i + 1]}.running_var"]
